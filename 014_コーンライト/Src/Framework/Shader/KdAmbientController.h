...
struct KdAmbientParameter
{
	...
	float m_coneLightRange = 0.f;
	float m_coneLightAngle = 0.f;
...
public:
	...
	// 黒崎授業
	// コーンライト有効化
	void SetConeLightEnable(const bool _enable);
	// コーンライト設定
	void SetConeLight(const Math::Vector3& _pos, const Math::Vector3& _dir, const float& _range, const float& _angle, const Math::Vector3& _col = { 3.f,3.f,3.f });
