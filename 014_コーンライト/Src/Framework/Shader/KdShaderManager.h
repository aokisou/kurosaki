...
struct ConeLight
{
	...
	float range = 0;						// 長さ
	float angle = 0;						// 角度
	Math::Vector3 color = { 3.f,3.f,3.f };	// 色
};
...
public:
	...
	// 黒崎授業
	void WrightCBConeLightEnable(const bool _enable);
	void WrightCBConeLight(const Math::Vector3& _pos, const Math::Vector3& _dir, const float& _range, const float& _angle, const Math::Vector3& _col = { 3.f,3.f,3.f });
