...
public:
	...
	// 光学迷彩有効化
	void SetOpticalCamoEnable(const bool _enable)
	{
		m_cb0_Obj.Work().OpticalCamoEnable = _enable;

		m_dirtyCBObj = true;
	}

	//　ゲーム画面テクスチャを設定する
	void SetScreenTexture(KdTexture& _tex)
	{
		KdDirect3D::Instance().WorkDevContext()->PSSetShaderResources(9, 1, _tex.WorkSRViewAddress());
	}
