...
void KdShaderManager::WriteCBColorEnable(const bool _enable)
{
...
void KdShaderManager::WriteCBColor(const Math::Vector3& _pos, const float& _radius, const Math::Vector3& _color)
{
	cbEffect& effect = m_cb10_Effect.Work();
	effect.ColorPos = _pos;
	effect.ColorRadius = _radius;
	effect.Color = _color;
